...
void* defProcAlloc( void *ud, void *prev, size_t size, size_t want )
{
	return realloc( prev, want );
}

cbAlloc procAlloc = defProcAlloc;

struct mapped* dump_maps
(
	void *ud
	, FILE *in
	, struct glance_at_maps *glance_at_maps
)
{
	int ret = 0;
	char *path, name[bitsof(void*)] = {0}, *text, *line, *eol;
	void *buf = NULL, *tmp;
	FILE *maps, *out = NULL;
	size_t size = 0, path_size = 0, want, got = BUFSIZ, did, text_size;
	struct mapped *list, *mapped;
		
	want = bitsof(void*) + (dir ? strlen(dir) + 1 : 1);
	path = inc_memory_group_total( ud, glance_at_maps->path, want );
	...
	/* Get the list and get out */
	for ( want = 0; got == BUFSIZ; want += BUFSIZ  )
	{	
		text = inc_memory_block
		(
			ud
			, glance_at_maps->buff
			, want + BUFSIZ
		...
		line = text + want;
		got = fread( line, BUFSIZ, 1, in );
		
		/* Make sure details can be read later after mappings have already
		 * changed, side effect of enabling non-rooted app to read what the
		 * state was at the time it was glanced at */
		if ( got )
		{
			did = fwrite( line, got, 1, out );
			if ( did != got )
				break;
		}
	}
	...
	/* Prepare our list elements but don't be ridiculous about memory */
	want = glance_at_maps->path->memory_block.bytes;
	want = sizeof(struct mapped) * ((want / sizeof(struct mapped))+1);
	list = inc_memory_block( ud, &(glance_at_maps->list), want );
	
	if ( !list )
		return ENOMEM;
	
	/* Analyse the text we read */
	for ( line = glance_at_maps->buff->block; *line; )
	{
		sscanf
		(
			line
		...
		/* Find where the end of the mapped path lies (if there is one) */
		eol = line;
		for ( ; *eol && *eol != '\n'; ++eol );
		
		/* Find where the start of the mapped path lies (if there is one) */
		text = eol;
		for ( ; *text != '\t'; --text );
		
		/* Ensure the path can be read easily */
		if ( text != eol )
		{
			want = (uintptr_t)(eol) - (uintptr_t)(text);
			buff = inc_memory_group_total( ud, &(mapped->msrc), want + 1 );
				
			if ( !buff )
				return ENOMEM;
				
			memcpy( buff, text, want );
		}
